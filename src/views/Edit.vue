...
export default {
  ...
  methods: {
    ...
    async uploadCover($event) {
      ...
      const res = await request({ method: 'post', url: '/upload', data: param, headers });
      this.coverUrl = `/upload/${res}`
    },
    ...
    async $imgAdd(pos, $file) {
      ...
      const headers = { "Content-Type": "multipart/form-data" };
      const result = await request({ method: 'post', url: '/upload', data: formdata, headers });
      // 将图片地址返回
      const url = `/upload/${result}`;
